...
def process_pdf_universal_standalone_engine(pdf_file, password=None):
    """11zon-style Clean Bounding Box Parser with Strict Metadata Filtering"""
    ...
    ignore_phrases = [
        ...
        "phone no", "generated on", "generated by", "this is a computer generated", 
        ...
        "currency email", "godown no", "najafgarh road"
    ]
    ...
    with pdfplumber.open(pdf_file, password=password if password else None) as pdf:
        for page in pdf.pages:
            ...
            for line_y in sorted(lines_dict.keys()):
                ...
                # Filter Header Metadata Rows completely
                ...
                # Date MUST be in the Date Column (x_rel < 0.12)
                ...
                if d_match or ((dr_amt > 0 or cr_amt > 0) and current_tx is not None):
                    ...
                    current_tx = {
                        "Date": d_match.group(0) if d_match else (current_tx["Date"] if current_tx else "01/04/2025"),
                        "Narration": full_narr,
...
with tab3:
    ...
    st.write("Kisi bhi Bank ki Borderless ya Complex PDF statement ko directly clean Excel sheet mein convert karein (With Live Screen Editor).")
    ...
    if uploaded_standalone_pdf is not None:
        with st.spinner("⌛ Processing PDF with Bounding Box Engine..."):
